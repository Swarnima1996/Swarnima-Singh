...
- **`created_pet`** — Used by tests that need a pre-existing pet to operate on (read, update, delete). It creates a pet before the test and deletes it in teardown, whether the test passes or fails.
- **`tracked_client`** — Used by tests that create pets themselves (e.g., `TestCreatePet`). It yields a `(client, created_ids)` tuple. Tests append each created pet's ID to `created_ids`, and all registered IDs are deleted in teardown. This is used whenever a test calls `client.add_pet(...)` directly rather than relying on a pre-built pet.
This approach ensures that tests never share state through the API, a failed test does not leave data that affects the next one, and tests can be executed in any order.
